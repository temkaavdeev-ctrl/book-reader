...
`is_member()` is admin **or** a `memberships` row **or** email on `whitelist`. A session token alone is not membership.
...
Post-auth home is `path.html` (Путь), not `solve-home.html`.
...
- Deploy remains owner-irreversible (`deploy_stage.sh` in `book_sources`, `deploy_candidates`).
