...
const mcAuth = fs.readFileSync(path.join(ROOT, 'mc-auth.js'), 'utf8');
const sql = fs.readFileSync(path.join(ROOT, 'sql/membership_after_auth.sql'), 'utf8');
const pathPage = fs.readFileSync(path.join(ROOT, 'path.html'), 'utf8');
...
// Identity ≠ membership on Моё and in the shared organ.
has(/STATE=\{signedIn:false,\s*member:false/, personal, 'STATE has signedIn and member');
has(/rpc\('my_access'\)/, mcAuth, 'membership comes from my_access');
has(/rpc\('is_member'\)/, mcAuth, 'my_access falls back to is_member');
...
refute(/if\(mcLikelySession\(\)\) global\.__MEMBER=true/, mcAuth, 'session token must not set __MEMBER');
has(/redirect:'path\.html'/, mcAuth, 'post-auth home is path.html');
has(/index\.html/, pathPage, 'path.html aliases the reader monolith');
refute(/auth\/v1\/authorize/i, pathPage, 'path.html must not use raw authorize URL');
